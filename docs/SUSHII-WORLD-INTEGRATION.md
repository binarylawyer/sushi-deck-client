...
The kit also owns the three versioned watercolor assets under `examples/sushii-world/assets/`. The client must **not** carry a second copy of the presentation or artwork in `public/`. Instead, `/sushii-world` and `/sushii-world/assets/*` resolve the installed kit package so the browser receives the exact version installed with the application.
